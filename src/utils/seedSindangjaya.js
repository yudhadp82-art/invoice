...
export function runSindangjayaSeeder() {
  ...
  const SINDANGJAYA_PRICES = [
    ...
    { "name": "sania 18ltr", "price": 380500, "unit": "jrigen" },
    ...
    { "name": "kacang tolo", "price": 30000, "unit": "jrigen" },
    ...
    { "name": "kunyit giling", "price": 26000, "unit": "kg" },
    { "name": "bunga teleng", "price": 75000, "unit": "kg" },
  ...
  for (const item of SINDANGJAYA_PRICES) {
    if (item.price === 0) continue; // Skip items without a price
    ...
    if (existingProduct) {
      // Update existing item's category price
      ...
      Products.create({
        name: item.name,
        sku: newSku,
        category: 'Sayuran / Bahan Pokok', // Defaulting to this
        purchaseCost: Math.floor(item.price * 0.8), // mock purchase cost
        sellPrice: item.price, // Default sell price, could be slightly higher
        stock: 0,
        unit: item.unit,
